...
extern "C" fn kmain() -> ! {
    ...
    COM1.init();
    klog!("GLM OS v1.2.0 (x86_64, long mode, SMP, threads, networking, gui, ring-3 windows) kernel entry");
    ...
    console::print("   v1.2.0  x86_64 long mode  SMP + networking + ring3 + gui windows");
    console::newline();
    console::newline();
    ...
    // --- shell ----------------------------------------------------------------
    console::set_color_global(GLM_WHITE);
    console::print("  GLM OS v1.2.0 ready.");
    console::set_color_global(GLM_GRAY);
    console::newline();
